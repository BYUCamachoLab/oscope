...
try:
    ...
    scope.write('ACQ:SRAT 20e9') #Define the number of samples per second
    ...
    # Trigger Settings:
    # -----------------------------------------------------------
    scope.write('TRIG1:MODE NORMal')  # Trigger Auto mode in case of no signal is applied
    ...
    scope.write('TRIG1:LEV2:VAL 2')  # Trigger level 2V
    ...
    waveformASCII = scope.query_ascii_values('FORM ASC;:CHAN2:DATA?')
    print('ASCII data samples read: {}'.format(len(waveformASCII)))

    print('Writing data samples to file.')
    with open(Path.cwd() /'Test Data'/'data.txt', 'w') as dataOut:
    ...
    scope.ext_query_bin_data_to_file('MMEM:DATA? \'c:\\temp\\Dev_Screenshot.png\'', Path.cwd() /'Test Data'/'Screenshot.png')
